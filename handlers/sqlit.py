...
def reg_user(id):
    ...
    if sql.fetchone() is None:
        sql.execute(f"INSERT INTO user_time VALUES (?,?)", (id,'wait'))
        db.commit()
...
def members_list():
    ...
    ru = sql.execute(f"SELECT COUNT(*) FROM user_time WHERE country = 'ru'" ).fetchone()[0]
    bel = sql.execute(f"SELECT COUNT(*) FROM user_time WHERE country = 'bel'").fetchone()[0]
    uk = sql.execute(f"SELECT COUNT(*) FROM user_time WHERE country = 'uk'").fetchone()[0]
    another_country = sql.execute(f"SELECT COUNT(*) FROM user_time WHERE country = 'another_country'").fetchone()[0]
    kz = sql.execute(f"SELECT COUNT(*) FROM user_time WHERE country = 'kz'").fetchone()[0]
    kr = sql.execute(f"SELECT COUNT(*) FROM user_time WHERE country = 'kr'").fetchone()[0]
    uzb = sql.execute(f"SELECT COUNT(*) FROM user_time WHERE country = 'uzb'").fetchone()[0]
    wait = sql.execute(f"SELECT COUNT(*) FROM user_time WHERE country = 'wait'").fetchone()[0]

    return [all, kz,kr,uzb,ru,uk,bel,another_country,wait]
